...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) // Last Updated 19/01/2013
{
	...
	ServerIndex = openwyd_env_int("OPENWYD_SERVER_GROUP", 0);
...
#endif
		DB_PORT, WSA_ACCEPT);
	AdminSocket.StartListen(hWndMain,
#ifdef OPENWYD_HEADLESS
		bindIp,
#else
		0,
#endif
		ADMIN_PORT, WSA_ACCEPTADMIN);
	...
	case WSA_ACCEPT:
		{
			...
			for(int i = 0; i < MAX_SERVER; i++)
			{
				if(pUser[i].IP == TempUser.IP && pUser[i].Mode != USER_EMPTY)
				{
					pUser[i].cSock.CloseSocket();
					pUser[i].Mode = USER_EMPTY;
					User = i;
					break;
				}
			}
...
#ifdef OPENWYD_HEADLESS
			const bool trustedGameServer = true;
#else
			const bool trustedGameServer =
				cIP[0] == LocalIP[0] &&
				cIP[1] == LocalIP[1] &&
				cIP[2] == LocalIP[2];
#endif
			if(trustedGameServer)
			{
