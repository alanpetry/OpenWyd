...
void DrawConfig(int wb)
{
	...
	const char* gambiZero1 = warsTimer[eTower].Minute < 10 ? "0" : "";
	SetTextColor(hDC, backcolor);
	...
	y += 16;
	const char* torreSunday = warsTimer[eTower].Days[Sunday] ? "X" : " ";
	const char* torreMonday = warsTimer[eTower].Days[Monday] ? "X" : " ";
	const char* torreTuesday = warsTimer[eTower].Days[Tuesday] ? "X" : " ";
	const char* torreWednesday = warsTimer[eTower].Days[Wednesday] ? "X" : " ";
	const char* torreThursday = warsTimer[eTower].Days[Thursday] ? "X" : " ";
	const char* torreFriday = warsTimer[eTower].Days[Friday] ? "X" : " ";
	const char* torreSaturday = warsTimer[eTower].Days[Saturday] ? "X" : " ";
	...
	const char* gambiZero2 = warsTimer[eNoatum].Minute < 10 ? "0":"";
	...
	y += 16;
	const char* noatumSunday = warsTimer[eNoatum].Days[Sunday] ? "X" : " ";
	const char* noatumMonday = warsTimer[eNoatum].Days[Monday] ? "X" : " ";
	const char* noatumTuesday = warsTimer[eNoatum].Days[Tuesday] ? "X" : " ";
	const char* noatumWednesday = warsTimer[eNoatum].Days[Wednesday] ? "X" : " ";
	const char* noatumThursday = warsTimer[eNoatum].Days[Thursday] ? "X" : " ";
	const char* noatumFriday = warsTimer[eNoatum].Days[Friday] ? "X" : " ";
	const char* noatumSaturday = warsTimer[eNoatum].Days[Saturday] ? "X" : " ";
	...
	const char* gambiZero3 = warsTimer[eRvR].Minute < 10 ? "0" : "";
	...
	y += 16;
	const char* rvrSunday = warsTimer[eRvR].Days[Sunday] ? "X" : " ";
	const char* rvrMonday = warsTimer[eRvR].Days[Monday] ? "X" : " ";
	const char* rvrTuesday = warsTimer[eRvR].Days[Tuesday] ? "X" : " ";
	const char* rvrWednesday = warsTimer[eRvR].Days[Wednesday] ? "X" : " ";
	const char* rvrThursday = warsTimer[eRvR].Days[Thursday] ? "X" : " ";
	const char* rvrFriday = warsTimer[eRvR].Days[Friday] ? "X" : " ";
	const char* rvrSaturday = warsTimer[eRvR].Days[Saturday] ? "X" : " ";
	...
	const char* gambiZero4 = warsTimer[eCity].Minute < 10 ? "0" : "";
	...
	y += 16;
	const char* citySunday = warsTimer[eCity].Days[Sunday] ? "X" : " ";
	const char* cityMonday = warsTimer[eCity].Days[Monday] ? "X" : " ";
	const char* cityTuesday = warsTimer[eCity].Days[Tuesday] ? "X" : " ";
	const char* cityWednesday = warsTimer[eCity].Days[Wednesday] ? "X" : " ";
	const char* cityThursday = warsTimer[eCity].Days[Thursday] ? "X" : " ";
	const char* cityFriday = warsTimer[eCity].Days[Friday] ? "X" : " ";
	const char* citySaturday = warsTimer[eCity].Days[Saturday] ? "X" : " ";
	...
	const char* aMin0 = altarKing.Min[0] < 10 ? "0" : "";
	const char* aMin1 = altarKing.Min[1] < 10 ? "0" : "";
	const char* aMin2 = altarKing.Min[2] < 10 ? "0" : "";

	const char* rSunday = altarKing.Days[Sunday] ? "X" : " ";
	const char* rMonday = altarKing.Days[Monday] ? "X" : " ";
	const char* rTuesday = altarKing.Days[Tuesday] ? "X" : " ";
	const char* rWednesday = altarKing.Days[Wednesday] ? "X" : " ";
	const char* rThursday = altarKing.Days[Thursday] ? "X" : " ";
	const char* rFriday = altarKing.Days[Friday] ? "X" : " ";
	const char* rSaturday = altarKing.Days[Saturday] ? "X" : " ";
...
BOOL WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	const char* bindAddress = openwyd_env_string("OPENWYD_BIND_ADDRESS", "0.0.0.0");
	unsigned int bindIp = inet_addr(bindAddress);
	if (bindIp == INADDR_NONE)
		bindIp = INADDR_ANY;
	memcpy(LocalIP, &bindIp, sizeof(LocalIP));
	strcpy(DBServerAddress, openwyd_env_string("OPENWYD_DB_HOST", "dbsrv"));
	DBServerPort = openwyd_env_int("OPENWYD_DB_PORT", 7514);
	ServerGroup = openwyd_env_int("OPENWYD_SERVER_GROUP", 0);
	ServerIndex = openwyd_env_int("OPENWYD_SERVER_INDEX", 0);
	FILE* fp = NULL;
#else
	char name[255];
	...
	ListenSocket.StartListen(hWndMain,
#ifdef OPENWYD_HEADLESS
		bindIp,
#else
		*pip,
#endif
		GAME_PORT, WSA_ACCEPT);
...
LONG APIENTRY MainWndProc(HWND hWnd, UINT message, UINT wParam, LONG lParam)
{
	...
	case WM_CREATE:
	{
		...
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&File");

		hSubMenu = CreatePopupMenu();
		AppendMenu(hSubMenu, MF_STRING, IDC_MOBRELOAD, "&Load Mob");
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Mob");
		...
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Sistema");
		...
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Events");

		hSubMenu = CreatePopupMenu();
		AppendMenu(hSubMenu, MF_STRING, IDC_READSANCRATE, "&Load Sanc");
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Rates");
		...
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Config Extra");

		hSubMenu = CreatePopupMenu();
		AppendMenu(hSubMenu, MF_STRING, IDC_CONVERT_NPC, "&Convert NPC to Json");
		AppendMenu(hMenu, MF_STRING | MF_POPUP, (uintptr_t)hSubMenu, "&Tools");
...
void Combine(int conn, int item, int PrizeItem)
{
	...
	if (i >= 0 && i < MAX_CARRY)
	{
		memcpy(&pMob[conn].MOB.Carry[i], &Prize, sizeof(STRUCT_ITEM));
	}
