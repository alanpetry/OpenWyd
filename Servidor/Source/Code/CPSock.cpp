...
template <typename T>
static T ReadUnaligned(const char* source)
{
	T value;
...
static unsigned int ResolveIPv4(const char* host)
{
	unsigned int address = inet_addr(host);
...
SOCKET CPSock::ConnectServer(char *HostAddr, int Port, int ip, int WSA)
{
	...
	InAddr.sin_addr.s_addr = ResolveIPv4(HostAddr);
...
SOCKET CPSock::ConnectBillServer(char *HostAddr, int Port, int ip, int WSA)
{
	...
	sprintf(msg, "sock:%d ip:%s port:%d-%d", tSock, inet_ntoa(local_sin.sin_addr),
		ntohs(local_sin.sin_port), ConnectPort);

	ConnectPort++;

	remote_sin.sin_addr.s_addr = ResolveIPv4(HostAddr);
	if (remote_sin.sin_addr.s_addr == INADDR_NONE)
	{
		closesocket(tSock);
		return 0;
	}
	...
	{
		sprintf(msg, "sock:%d ip:%s port:%d-%d", tSock, inet_ntoa(remote_sin.sin_addr),
			ntohs(remote_sin.sin_port), Port);
...
BOOL CPSock::Receive()
{
	...
	if (tReceiveSize <= 0 || tReceiveSize == Rest)
		return FALSE;
...
char* CPSock::ReadMessage(int *ErrorCode, int *ErrorType)
{
	*ErrorCode = 0;
	...
	if (Init == 0)
	{
		...
		unsigned int InitCode = ReadUnaligned<unsigned int>(
			pRecvBuffer + nProcPosition);
	...
	unsigned short	Size = ReadUnaligned<unsigned short>(
		pRecvBuffer + nProcPosition);
	unsigned char	iKeyWord = ReadUnaligned<unsigned char>(
		pRecvBuffer + nProcPosition + 2);
	unsigned char	KeyWord = pKeyWord[iKeyWord * 2];
	unsigned char	CheckSum = ReadUnaligned<unsigned char>(
		pRecvBuffer + nProcPosition + 3);
	unsigned int	SockType = ReadUnaligned<unsigned int>(
		pRecvBuffer + nProcPosition + 4);
	unsigned int	SockID = ReadUnaligned<unsigned int>(
		pRecvBuffer + nProcPosition + 6);
	(void)SockType;
	(void)SockID;
