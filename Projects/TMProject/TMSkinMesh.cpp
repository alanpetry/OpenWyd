	...
	int parentFound)
{
	...
	if (g_wydSkinRestoreLogCount >= 160)
		return;
	++g_wydSkinRestoreLogCount;

	char buf[512]{};
	std::snprintf(
		buf,
		sizeof(buf),
		"[skin:restore] stage=%s bone=%d meshType=%d mount=%d part=%u mesh=%s tex=%s load=%d id=%u parent=%d texIndex=%d influence=%u palette=%u faces=%u",
		stage ? stage : "?",
		skin ? skin->m_nBoneAniIndex : -1,
		skin ? static_cast<int>(skin->m_nMeshType) : -1,
	...
	for (unsigned int i = 0; i < MeshManager::m_BoneAnimationList[m_nBoneAniIndex].numParts; ++i)
	{
		...
		if ((int)* look < 90 || !i || look[2 * i])
		{
			...
			case 125:
...
#if defined(__EMSCRIPTEN__)
					WydLogSkinRestore("hand-frame", this, i, szName, szTexture, tmpMesh, 1, parent != nullptr);
#endif
					if (parent != nullptr)
					{
				...
				{
					const int loadResult = tmpMesh->LoadMesh(szName);
					CFrame* parent = loadResult == 1 ? m_pRoot->FindFrame(tmpMesh->m_dwID) : nullptr;
#if defined(__EMSCRIPTEN__)
					WydLogSkinRestore("load-mesh", this, i, szName, szTexture, tmpMesh, loadResult, parent != nullptr);
#endif
					if (loadResult == 1)
					{
						if (parent)
