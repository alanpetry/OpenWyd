...
void WasmInitLog(const char* msg)
{
#if defined(__EMSCRIPTEN__)
...
void WasmApplyCanvasResolution(unsigned int& width, unsigned int& height, unsigned int& colorBit)
{
	...
	width = static_cast<unsigned int>(canvasWidth);
	height = static_cast<unsigned int>(canvasHeight);
	colorBit = 32;
...
HRESULT NewApp::Initialize(HINSTANCE hInstance, int nFull)
{
	...
	{
		WasmInitLog("[newapp:init] BASE_InitializeBaseDef failed");
		MessageBoxA(m_hWnd, "Initialize Data Failed", "Error", 0);
		LOG_WRITELOGSTRING("Initialize Data Failed\r\n");
		return 0;
	}
	WasmInitLog("[newapp:init] BASE_InitializeBaseDef ok");

	ReadItemicon();
	ReadItemName();
	ReadUIString();
	BASE_ReadItemPrice();
	WasmInitLog("[newapp:init] item/ui data loaded");
	...
	if (fp == nullptr)
	{
		WasmInitLog("[newapp:init] itemhelp open failed");
	...
	fclose(fp);
	WasmInitLog("[newapp:init] itemhelp loaded");

	if (m_pAviPlayer)
		return 1;
	else
	{
		WasmInitLog("[newapp:init] calling InitDevice");
		return InitDevice();
	}
...
{
	WasmInitLog("[newapp:init-device] begin");
	...
	{
		WasmInitLog("[newapp:init-device] render initialize failed");
	...
	WasmInitLog("[newapp:init-device] render initialize ok");

	if (m_nSound > 0)
	{
		WasmInitLog("[newapp:init-device] sound setup");
		InitMusicList();
	...
	if (m_nMusic > 0)
	{
		WasmInitLog("[newapp:init-device] bgm setup");
		m_pBGMManager = new DS_SOUND_MANAGER(1, 30 * m_nMusic - 3000);
		g_pApp->m_pBGMManager->SetVolume(0, g_pApp->m_pBGMManager->m_lBGMVolume);
	}

	WasmInitLog("[newapp:init-device] event translator allocate");
	m_pEventTranslator = new EventTranslator();
	WasmInitLog("[newapp:init-device] event translator init");
	if (!m_pEventTranslator->Initialize(m_hWnd))
	{
		WasmInitLog("[newapp:init-device] event translator init failed");
		LOG_WRITELOG("Initialize Interface Failed");
		return 0;
	}

	WasmInitLog("[newapp:init-device] event translator ime");
	if (!m_pEventTranslator->InitializeIME())
	{
		WasmInitLog("[newapp:init-device] IME init failed");
		LOG_WRITELOG("Initialize IME Failed");
		return 0;
	}

	WasmInitLog("[newapp:init-device] timer/object manager create");
	m_pTimerManager = new TimerManager();

	m_pObjectManager = new ObjectManager();
	if (m_pObjectManager != nullptr)
	{
		WasmInitLog("[newapp:init-device] object manager restore");
	...
	{
		WasmInitLog("[newapp:init-device] timer start");
		m_pTimerManager->StartTimer();
	}

	WasmInitLog("[newapp:init-device] socket setup");
	m_pSocketManager = new CPSock();

	if (!m_pSocketManager->WSAInitialize())
	{
		WasmInitLog("[newapp:init-device] socket init failed");
		LOG_WRITELOG("Initialize Socket Failed");
		return 0;
	}
	g_pSocketManager = m_pSocketManager;
	g_LoginSocket = new CPSock();
	WasmInitLog("[newapp:init-device] login socket allocated");
	...
	m_pBlur = new JBlur();
	WasmInitLog("[newapp:init-device] blur init");

	int blurOk = (m_pBlur->InitObject() != 0);
	WasmInitLog(blurOk ? "[newapp:init-device] success" : "[newapp:init-device] blur init failed");
	return blurOk;
}
...
void NewApp::InitMusicList()
{
	...
	char szTemp1[256];
	for (int i = 0; i < 15 && fgets(szTemp1, sizeof(szTemp1), fp); ++i)
	{
		char szTemp2[256]{};
		const char* src = (std::strlen(szTemp1) > 17) ? &szTemp1[17] : szTemp1;
		std::snprintf(szTemp2, sizeof(szTemp2), "%s", src);

		size_t nLen = std::strlen(szTemp2);
		while (nLen > 0 && (szTemp2[nLen - 1] == '\n' || szTemp2[nLen - 1] == '\r'))
		{
			szTemp2[nLen - 1] = '\0';
			--nLen;
		}

		std::snprintf(DS_SOUND_MANAGER::m_szMusicPath[i], sizeof(DS_SOUND_MANAGER::m_szMusicPath[i]), "%s", szTemp2);
	}
...
{
	if (!m_hAccel)
		m_hAccel = LoadAccelerators(0, MAKEINTRESOURCE(IDC_TMPROJECT));
...
DWORD NewApp::RunTick(MSG* pMsg)
{
	...
	else
	{
		if (m_bActive == 1)
		{
...
#if defined(__EMSCRIPTEN__)
			TraceTick("[newapp:tick] input read");
#endif
			m_pEventTranslator->ReadInputEventData();
#if defined(__EMSCRIPTEN__)
				TraceTick("[newapp:tick] camera input");
#endif
				...
				if ((m_pAviPlayer == nullptr || m_pAviPlayer != nullptr && m_pAviPlayer->m_psCurrent != PLAYSTATE::Running)
					...
					&& !m_pRenderDevice->m_bLoadMeshManager)
				{
					TraceTick("[newapp:tick] init mesh manager before frame move");
					if (!m_pRenderDevice->InitMeshManager())
					{
						WasmInitLog("[newapp:tick] wasm mesh manager init failed");
			...
			{
#if defined(__EMSCRIPTEN__)
				TraceTick("[newapp:tick] render scene");
#endif
				RenderScene();
			}
			if ((m_pAviPlayer == nullptr || m_pAviPlayer != nullptr && m_pAviPlayer->m_psCurrent != PLAYSTATE::Running) &&
			...
			{
#if defined(__EMSCRIPTEN__)
				TraceTick("[newapp:tick] init mesh manager");
#endif
#if defined(__EMSCRIPTEN__)
				if (!m_pRenderDevice->m_bLoadMeshManager)
				{
					if (!m_pRenderDevice->InitMeshManager())
					{
						WasmInitLog("[newapp:tick] wasm mesh manager init failed");
			...
			{
#if defined(__EMSCRIPTEN__)
				TraceTick("[newapp:tick] object cleanup");
#endif
...
void CreateConsole()
{
	...
	// Output/input redirection for desktop runtime.
	FILE* out = nullptr;
	FILE* in = nullptr;
