...
#else
	// WASM bootstrap path: keep client runtime alive while DX9->WebGL2
	// compatibility layer is incrementally implemented. Skin meshes already
	// rely on vertex declarations for CPU-side shader replay in the bridge.
	if (!InitVertexShader())
	{
		LOG_WRITELOG("Error VertexShader \r\n");
		return 0;
