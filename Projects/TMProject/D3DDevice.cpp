...
int D3DDevice::Initialize(HWND hWnd)
{
	...
	std::memset(&m_d3dpp, 0, sizeof(m_d3dpp));
	m_bWindowed = true;
	...
	m_pd3dDevice = WydCreateDummyD3DDevice();
	if (!m_pd3dDevice)
		return DisplayErrorMsg(E_OUTOFMEMORY, MSGERR_APPMUSTEXIT);

	std::memset(&m_d3dCaps, 0, sizeof(m_d3dCaps));
	m_d3dCaps.DevCaps = D3DDEVCAPS_HWTRANSFORMANDLIGHT;
	m_d3dCaps.PresentationIntervals = D3DPRESENT_INTERVAL_DEFAULT | D3DPRESENT_INTERVAL_ONE | D3DPRESENT_INTERVAL_IMMEDIATE;
	m_d3dCaps.MaxTextureBlendStages = 8;
	m_d3dCaps.MaxTextureWidth = 4096;
	m_d3dCaps.MaxVertexBlendMatrices = 35;
	m_d3dCaps.PixelShaderVersion = 0xFFFF0101;
	m_d3dCaps.VertexShaderVersion = 0xFFFE0200;

	m_d3dsdBackBuffer.Format = m_d3dpp.BackBufferFormat;
	m_d3dsdBackBuffer.Width = m_d3dpp.BackBufferWidth;
	m_d3dsdBackBuffer.Height = m_d3dpp.BackBufferHeight;
	m_d3dsdBackBuffer.Type = D3DRTYPE_SURFACE;
	m_d3dsdBackBuffer.MultiSampleType = D3DMULTISAMPLE_NONE;
	m_d3dsdBackBuffer.MultiSampleQuality = 0;

	m_dwCreateFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
	m_bDeviceObjectsInited = true;
	m_bDeviceObjectsRestored = true;

	lstrcpy(m_strDeviceStats, TEXT("WASM D3D9 shim"));
	return S_OK;
#else
	HRESULT hr;
