...
extern "C" float wyd_debug_demo_camera_offset_x();
extern "C" float wyd_debug_demo_camera_offset_y();
extern "C" float wyd_debug_demo_camera_offset_z();
extern "C" float wyd_debug_demo_camera_offset_h();
extern "C" float wyd_debug_demo_camera_offset_v();
static constexpr unsigned int kWydDebugDisableDemoCamera = 1u << 22;

static unsigned int g_wydDebugCameraValid = 0;
static unsigned int g_wydDebugCameraStandalone = 0;
static float g_wydDebugCameraX = 0.0f;
static float g_wydDebugCameraY = 0.0f;
...
static void WydRecordDebugCamera(TMCamera* pCamera)
{
	if (!pCamera)
	{
		g_wydDebugCameraValid = 0;
		return;
	}

	g_wydDebugCameraValid = 1;
	g_wydDebugCameraStandalone = static_cast<unsigned int>(pCamera->m_bStandAlone);
	g_wydDebugCameraX = pCamera->m_cameraPos.x;
	g_wydDebugCameraY = pCamera->m_cameraPos.y;
...
{
	if (!pCamera || !wyd_debug_demo_camera_offset_enabled())
		return;

	pCamera->m_cameraPos.x += wyd_debug_demo_camera_offset_x();
	pCamera->m_cameraPos.y += wyd_debug_demo_camera_offset_y();
	pCamera->m_cameraPos.z += wyd_debug_demo_camera_offset_z();
	pCamera->m_fHorizonAngle += wyd_debug_demo_camera_offset_h();
	pCamera->m_fBackHorizonAngle = pCamera->m_fHorizonAngle;
	pCamera->m_fVerticalAngle += wyd_debug_demo_camera_offset_v();
	pCamera->m_fBackVerticalAngle = pCamera->m_fVerticalAngle;
}

extern "C" unsigned int wyd_debug_camera_valid() { return g_wydDebugCameraValid; }
extern "C" unsigned int wyd_debug_camera_standalone() { return g_wydDebugCameraStandalone; }
extern "C" float wyd_debug_camera_x() { return g_wydDebugCameraX; }
extern "C" float wyd_debug_camera_y() { return g_wydDebugCameraY; }
...
int TMScene::FrameMove(unsigned int dwServerTime)
{
	...
	if (m_sPlayDemo >= 0
#if defined(__EMSCRIPTEN__)
		&& ((wyd_d3d9_get_debug_flags() & kWydDebugDisableDemoCamera) == 0u)
#endif
		)
		CameraAction();
...
void TMScene::CameraAction()
{
	...
	{
#if defined(__EMSCRIPTEN__)
		WydRecordDebugCamera(g_pObjectManager ? g_pObjectManager->m_pCamera : nullptr);
#endif
		return;
	}
...
#if defined(__EMSCRIPTEN__)
		WydApplyDebugDemoCameraOffset(pCamera);
		WydRecordDebugCamera(pCamera);
#endif
		return;
	}
