...
static bool WydDebugSkipCharSelectMesh()
{
	return (wyd_d3d9_get_debug_flags() & (1u << 19)) != 0;
}
#endif
#include "TMGlobal.h"
...
int TMObject::Render()
{
	...
	if (!IsVisible())
		return 1;
#if defined(__EMSCRIPTEN__)
	if (m_dwObjType == 1562 && WydDebugSkipCharSelectMesh())
		return 1;
#endif
	...
	if (g_pDevice->m_bVoodoo == 1)
	{
		...
		std::snprintf(
			debugScope,
			sizeof(debugScope),
			"TMObject type=%u pos=%.2f,%.2f h=%.2f angle=%.3f alpha=%d voodoo=1",
	...
	std::snprintf(
		debugScope,
		sizeof(debugScope),
		"TMObject type=%u pos=%.2f,%.2f h=%.2f angle=%.3f alpha=%d alphaObj=%d",
		static_cast<unsigned int>(m_dwObjType),
		m_vecPosition.x,
		m_vecPosition.y,
		m_fHeight,
