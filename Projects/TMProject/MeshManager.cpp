...
static constexpr unsigned int kWydDebugLogCommonMesh = 1u << 30;

static bool WydIsWasmHeapPointer(const void* ptr)
{
	if (ptr == nullptr)
		return false;

	const uintptr_t address = reinterpret_cast<uintptr_t>(ptr);
	return address < static_cast<uintptr_t>(emscripten_get_heap_size());
}

static void WydLogCommonMeshPointer(const char* tag, int nIndex, int bEffect, const char* fileName, const void* ptr)
{
	...
	std::snprintf(
		buf,
		sizeof(buf),
		"[mesh:%s] index=%d effect=%d ptr=%p heap=%zu file=%s",
		tag ? tag : "ptr",
		nIndex,
		bEffect,
		ptr,
		static_cast<size_t>(emscripten_get_heap_size()),
...
static char g_szCommonMeshFileNameBackup[MAX_COMMON_MESH][64]{};
...
static bool WydCommonMeshPathLooksLoadable(const char* fileName)
{
	if (!fileName || !fileName[0])
		return false;

	const char* dot = strrchr(fileName, '.');
	return dot != nullptr && !_stricmp(dot, ".msa");
}

static void WydRestoreCommonMeshSlot(MeshManager* manager, int nIndex, const char* reason)
{
	if (!manager || nIndex < 0 || nIndex >= MAX_COMMON_MESH)
		return;

	if (!g_szCommonMeshFileNameBackup[nIndex][0])
		return;

	if (!strcmp(manager->m_stCommonMesh[nIndex].szFileName, g_szCommonMeshFileNameBackup[nIndex]))
		return;

#if defined(__EMSCRIPTEN__)
	WydLogCommonMeshPointer(
		reason ? reason : "restore-common-name",
		nIndex,
		0,
		manager->m_stCommonMesh[nIndex].szFileName,
		manager->m_stCommonMesh[nIndex].pMesh);
#endif
	snprintf(
		manager->m_stCommonMesh[nIndex].szFileName,
		sizeof(manager->m_stCommonMesh[nIndex].szFileName),
		"%s",
		g_szCommonMeshFileNameBackup[nIndex]);
}
...
int MeshManager::InitMeshManager()
{
	...
	while (fscanf(fp, "%d %254s", &nIndex, szMeshName) == 2)
	{
		if (nIndex < 0 || nIndex >= MAX_COMMON_MESH)
			continue;

		snprintf(m_stCommonMesh[nIndex].szFileName, sizeof(m_stCommonMesh[nIndex].szFileName), "%s", szMeshName);
		snprintf(g_szCommonMeshFileNameBackup[nIndex], sizeof(g_szCommonMeshFileNameBackup[nIndex]), "%s", szMeshName);
	}
...
int MeshManager::LoadCommonMesh(int nIndex, int bEffect)
{
	...
	WydRestoreCommonMeshSlot(this, nIndex, "restore-before-load");

	if (!WydCommonMeshPathLooksLoadable(m_stCommonMesh[nIndex].szFileName))
		return 0;
...
TMMesh* MeshManager::GetCommonMesh(int nIndex, int bEffect, DWORD showTime)
{
	...
	WydRestoreCommonMeshSlot(this, nIndex, "restore-before-get");

#if defined(__EMSCRIPTEN__)
	if (m_stCommonMesh[nIndex].pMesh != nullptr && !WydIsWasmHeapPointer(m_stCommonMesh[nIndex].pMesh))
	{
		WydLogCommonMeshPointer("invalid-common-ptr", nIndex, bEffect, m_stCommonMesh[nIndex].szFileName, m_stCommonMesh[nIndex].pMesh);
		m_stCommonMesh[nIndex].pMesh = nullptr;
		WydRestoreCommonMeshSlot(this, nIndex, "restore-after-invalid-ptr");
	}
#endif
...
#if defined(__EMSCRIPTEN__)
	if (!WydIsWasmHeapPointer(m_stCommonMesh[nIndex].pMesh))
	{
		WydLogCommonMeshPointer("load-invalid-common-ptr", nIndex, bEffect, m_stCommonMesh[nIndex].szFileName, m_stCommonMesh[nIndex].pMesh);
		m_stCommonMesh[nIndex].pMesh = nullptr;
		return 0;
	}

	if ((wyd_d3d9_get_debug_flags() & kWydDebugLogCommonMesh) != 0u
		&& g_pCurrentScene
		&& g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_SELECT_SERVER)
	{
		char buf[160]{};
		std::snprintf(buf, sizeof(buf), "[mesh:get-common] index=%d effect=%d ptr=%p", nIndex, bEffect, static_cast<void*>(m_stCommonMesh[nIndex].pMesh));
		emscripten_console_log(buf);
	}
#endif
