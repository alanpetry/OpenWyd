...
namespace {
void WydSetCMeshDrawLabel(const CMesh* mesh, const TMMesh* drawMesh, const char* kind, int texIndex, char alpha, int bright, int partIndex)
{
    const TMSkinMesh* skin = mesh ? mesh->m_pParentSkin : nullptr;
    const TMHuman* owner = skin ? skin->m_pOwner : nullptr;
    char label[640]{};
    std::snprintf(
        label,
        sizeof(label),
        "CMesh kind=%s owner=%.15s ownerPos=%.2f,%.2f,%.2f skinPos=%.2f,%.2f,%.2f boneAni=%d skinMeshType=%d mount=%d meshIndex=%d meshId=%u part=%d tex=%d alpha=%c bright=%d influence=%u palette=%u faces=%u vertices=%u fvf=%u multi=%d legend=%d",
        kind ? kind : "unknown",
        owner ? owner->m_szName : "(no-owner)",
        owner ? owner->m_vecPosition.x : 0.0f,
        owner ? owner->m_fHeight : 0.0f,
        owner ? owner->m_vecPosition.y : 0.0f,
        skin ? skin->m_vPosition.x : 0.0f,
        skin ? skin->m_vPosition.y : 0.0f,
        skin ? skin->m_vPosition.z : 0.0f,
        skin ? skin->m_nBoneAniIndex : -1,
        skin ? skin->m_nMeshType : -1,
        skin ? skin->m_bMount : -1,
        mesh ? mesh->m_nSkinMeshIndex : -1,
        mesh ? static_cast<unsigned int>(mesh->m_dwID) : 0u,
        partIndex,
        texIndex,
        alpha ? alpha : '?',
        bright,
        mesh ? static_cast<unsigned int>(mesh->m_numFaceInflunce) : 0u,
        mesh ? static_cast<unsigned int>(mesh->m_numPalette) : 0u,
        mesh ? static_cast<unsigned int>(mesh->m_numFaces) : 0u,
        drawMesh ? static_cast<unsigned int>(drawMesh->m_AttRange[0].VertexCount) : 0u,
        drawMesh ? static_cast<unsigned int>(drawMesh->m_dwFVF) : 0u,
        mesh ? static_cast<int>(mesh->m_sMultiType) : 0,
        mesh ? static_cast<int>(mesh->m_sLegendType) : 0);
    wyd_d3d9_set_draw_label(label);
}

void WydSetCMeshDrawScope(const CMesh* mesh, const char* kind, int partIndex)
{
    const TMSkinMesh* skin = mesh ? mesh->m_pParentSkin : nullptr;
    const TMHuman* owner = skin ? skin->m_pOwner : nullptr;
    char label[384]{};
    std::snprintf(
        label,
        sizeof(label),
        "CMeshScope kind=%s owner=%.15s ownerPos=%.2f,%.2f,%.2f boneAni=%d meshId=%u part=%d skinMeshIndex=%d multi=%d legend=%d",
        kind ? kind : "unknown",
        owner ? owner->m_szName : "(no-owner)",
        owner ? owner->m_vecPosition.x : 0.0f,
        owner ? owner->m_fHeight : 0.0f,
...
CMesh::CMesh(TMSkinMesh* pParentSkin)
{
    ...
    m_dwNames = nullptr;
    std::memset(m_pBoneOffset, 0, sizeof(m_pBoneOffset));
...
#if defined(__EMSCRIPTEN__)
        WydSetCMeshDrawLabel(this, pMesh, "hand-special-2889", pMesh->m_nTextureIndex[0], cAlpha, nBright, nPartIndex);
#endif
...
#if defined(__EMSCRIPTEN__)
        WydSetCMeshDrawLabel(this, pMesh, "hand-special-2912", pMesh->m_nTextureIndex[0], cAlpha, nBright, nPartIndex);
#endif
        if (g_pDevice->m_pd3dDevice->DrawIndexedPrimitive(
            D3DPRIMITIVETYPE::D3DPT_TRIANGLELIST,
            0,
...
int CMesh::RenderMesh(char cAlpha)
{
            ...
            || m_dwID == g_dwHandIndex[m_pParentSkin->m_nBoneAniIndex][1]))
    {
        ...
        if (nPartIndex >= 13 && nPartIndex < 1000 || nPartIndex > 2700 && nPartIndex <= 3000)
        {
            ...
            {
#if defined(__EMSCRIPTEN__)
               WydSetCMeshDrawScope(this, "hand-common", nPartIndex);
#endif
...
#if defined(__EMSCRIPTEN__)
        WydSetCMeshDrawLabel(this, m_pMesh, "skin-main", m_nTextureIndex, cAlpha, -1, -1);
#endif
...
#if defined(__EMSCRIPTEN__)
            WydSetCMeshDrawLabel(this, m_pMesh, "skin-mouseover-edge", m_nTextureIndex, cAlpha, -1, -1);
#endif
