...
int BASE_InitializeServerList()
{
	...
	if (fpBin)
	{
		static constexpr unsigned char kLegacyServerListKey[64] = {
			0xA4, 0xA1, 0xA4, 0xA4, 0xA4, 0xA7, 0xA4, 0xA9,
			0xA4, 0xB1, 0xA4, 0xB2, 0xA4, 0xB5, 0xA4, 0xB7,
			0xA4, 0xB8, 0xA4, 0xBA, 0xA4, 0xBB, 0xA4, 0xBC,
			0xA4, 0xBD, 0xA4, 0xBE, 0xA4, 0xBF, 0xA4, 0xC1,
			0xA4, 0xC3, 0xA4, 0xC5, 0xA4, 0xC7, 0xA4, 0xCB,
			0xA4, 0xCC, 0xA4, 0xD0, 0xA4, 0xD1, 0xA4, 0xD3,
			0xA4, 0xBF, 0xA4, 0xC4, 0xA4, 0xD3, 0xA4, 0xC7,
			0xA4, 0xCC, 0xB0, 0xA1, 0xB3, 0xAA, 0xB4, 0xD9
		};
		static constexpr unsigned char kOfficialServerListKey[64] = {
			0xC1, 0xB6, 0xC0, 0xCC, 0xC0, 0xD3, 0xC6, 0xD1,
			0xC6, 0xAE, 0xBE, 0xCF, 0xC8, 0xA3, 0xC8, 0xAD,
			0xC0, 0xDB, 0xBE, 0xF7, 0xC0, 0xBB, 0xC0, 0xA7,
			0xC7, 0xD1, 0xBD, 0xBA, 0xC5, 0xA9, 0xB8, 0xB3,
			0xC6, 0xAE, 0xC0, 0xD4, 0xB4, 0xCF, 0xB4, 0xD9,
			0xB8, 0xB8, 0xC7, 0xD1, 0xB1, 0xDB, 0xB7, 0xCE,
			0xBE, 0xCF, 0xC8, 0xAD, 0xC8, 0xAD, 0xC7, 0xD2,
			0xC1, 0xD9, 0xA4, 0xBB, 0xA4, 0xBB, 0x00, 0x00
		};

		memset(&g_pServerList, 0, sizeof g_pServerList);
		fseek(fpBin, 0, SEEK_END);
		const long fileSize = ftell(fpBin);
		fseek(fpBin, 0, SEEK_SET);

		const bool hasOfficialHeader = fileSize >= static_cast<long>(sizeof g_pServerList + 4);
		if (hasOfficialHeader)
			fseek(fpBin, 4, SEEK_SET);

		fread(g_pServerList, 0x6E, 0x40u, fpBin);
		fclose(fpBin);

		const unsigned char* key = hasOfficialHeader ? kOfficialServerListKey : kLegacyServerListKey;
		for (int k = 0; k < MAX_SERVERGROUP; k++)
			for (int j = 0; j < MAX_SERVERNUMBER; j++)
				for (int i = 0; i < 64; i++)
					g_pServerList[k][j][i] = static_cast<char>(
						static_cast<unsigned char>(g_pServerList[k][j][i]) - key[63 - i]);
		return 1;
	}
