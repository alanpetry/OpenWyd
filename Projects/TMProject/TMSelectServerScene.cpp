...
struct WydSelServerHumanTelemetry
{
	...
	int mountAniCut;
	int mountGenerated;
	int mountFrameMeshes;
	int weaponTypeIndex;
	int headIndex;
	int bodyCurrentTable;
	int bodyCurrentResolvedClip;
	int bodyMountedCurrentTable;
	int bodyMountedCurrentResolvedClip;
	int bodySeatingTable;
	int bodySeatingResolvedClip;
	int bodyMountedSeatingTable;
	int bodyMountedSeatingResolvedClip;
	int demoAni;
	int moving;
	int sliding;
	int lastRouteIndex;
...
static const WydSelServerHumanTelemetry* WydSelServerHumanTelemetryAt(unsigned int index)
{
	if (index >= 50)
		return nullptr;
	return &g_wydSelServerHumanTelemetry[index];
}

extern "C" unsigned int wyd_selserver_human_version()
{
	return g_wydSelServerHumanTelemetryVersion;
}

extern "C" unsigned int wyd_selserver_human_count()
{
	return 50;
}

extern "C" unsigned int wyd_selserver_human_present(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->present : 0;
}

extern "C" float wyd_selserver_human_pos_x(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->posX : 0.0f;
}

extern "C" float wyd_selserver_human_pos_y(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->posY : 0.0f;
}

extern "C" float wyd_selserver_human_want_height(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->wantHeight : 0.0f;
}

extern "C" float wyd_selserver_human_height(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->height : 0.0f;
}

extern "C" float wyd_selserver_human_ground_mask(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->groundMask : 0.0f;
}

extern "C" float wyd_selserver_human_ground_height(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->groundHeight : 0.0f;
}

extern "C" float wyd_selserver_human_skin_x(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinX : 0.0f;
}

extern "C" float wyd_selserver_human_skin_y(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinY : 0.0f;
}

extern "C" float wyd_selserver_human_skin_z(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinZ : 0.0f;
}

extern "C" int wyd_selserver_human_skin_mesh_type(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinMeshType : 0;
}

extern "C" int wyd_selserver_human_obj_type(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->objType : 0;
}

extern "C" int wyd_selserver_human_visible(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->visible : 0;
}

extern "C" int wyd_selserver_human_mount(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mount : 0;
}

extern "C" int wyd_selserver_human_mount_skin_mesh_type(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountSkinMeshType : 0;
}

extern "C" int wyd_selserver_human_motion(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->motion : 0;
}

extern "C" int wyd_selserver_human_sent_motion(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
...
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinAni : 0;
}

extern "C" int wyd_selserver_human_skin_fps(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinFps : 0;
}

extern "C" int wyd_selserver_human_skin_offset(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinOffset : 0;
}

extern "C" int wyd_selserver_human_skin_bone_ani(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinBoneAni : 0;
}

extern "C" int wyd_selserver_human_skin_base_mat(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinBaseMat : 0;
}

extern "C" int wyd_selserver_human_skin_ani_base_index(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinAniBaseIndex : 0;
}

extern "C" int wyd_selserver_human_skin_ani_cut(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinAniCut : 0;
}

extern "C" int wyd_selserver_human_skin_generated(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinGenerated : 0;
}

extern "C" int wyd_selserver_human_skin_frame_meshes(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->skinFrameMeshes : 0;
}

extern "C" int wyd_selserver_human_mount_present(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountPresent : 0;
}

extern "C" int wyd_selserver_human_mount_ani(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountAni : 0;
}

extern "C" int wyd_selserver_human_mount_fps(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountFps : 0;
}

extern "C" int wyd_selserver_human_mount_offset(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountOffset : 0;
}

extern "C" int wyd_selserver_human_mount_ani_cut(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountAniCut : 0;
}

extern "C" int wyd_selserver_human_mount_generated(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountGenerated : 0;
}

extern "C" int wyd_selserver_human_mount_frame_meshes(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->mountFrameMeshes : 0;
}

extern "C" int wyd_selserver_human_weapon_type_index(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->weaponTypeIndex : 0;
}

extern "C" int wyd_selserver_human_head_index(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->headIndex : 0;
}

extern "C" int wyd_selserver_human_body_current_table(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyCurrentTable : 0;
}

extern "C" int wyd_selserver_human_body_current_resolved_clip(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyCurrentResolvedClip : 0;
}

extern "C" int wyd_selserver_human_body_mounted_current_table(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyMountedCurrentTable : 0;
}

extern "C" int wyd_selserver_human_body_mounted_current_resolved_clip(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyMountedCurrentResolvedClip : 0;
}

extern "C" int wyd_selserver_human_body_seating_table(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodySeatingTable : 0;
}

extern "C" int wyd_selserver_human_body_seating_resolved_clip(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodySeatingResolvedClip : 0;
}

extern "C" int wyd_selserver_human_body_mounted_seating_table(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyMountedSeatingTable : 0;
}

extern "C" int wyd_selserver_human_body_mounted_seating_resolved_clip(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->bodyMountedSeatingResolvedClip : 0;
}

extern "C" int wyd_selserver_human_demo_ani(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
...
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->progressRate : 0.0f;
}

extern "C" float wyd_selserver_human_max_speed(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->maxSpeed : 0.0f;
}

extern "C" int wyd_selserver_human_sliding(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
...
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->targetX : 0;
}

extern "C" int wyd_selserver_human_target_y(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->targetY : 0;
}

extern "C" float wyd_selserver_human_delta_x(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->deltaX : 0.0f;
}

extern "C" float wyd_selserver_human_delta_y(unsigned int index)
{
	auto entry = WydSelServerHumanTelemetryAt(index);
	return entry ? entry->deltaY : 0.0f;
}
#endif

#if defined(__EMSCRIPTEN__)
namespace {
void WasmSelServerLog(const char* msg)
{
	emscripten_console_log(msg ? msg : "(null)");
}

void WasmSelServerLogPath(const char* prefix, const char* path)
{
	char buf[256]{};
	std::snprintf(buf, sizeof(buf), "%s%s", prefix ? prefix : "", path ? path : "(null)");
	emscripten_console_log(buf);
}

int WasmSelServerCountFrameMeshes(CFrame* frame)
{
	int count = 0;
	for (CFrame* node = frame; node; node = node->m_pSibling)
	{
		if (node->m_pMesh)
			++count;
		if (node->m_pFirstChild)
			count += WasmSelServerCountFrameMeshes(node->m_pFirstChild);
	}
	return count;
}

int SelServerAniCut(const TMSkinMesh* skin, int aniIndex)
{
	if (!skin)
		return 0;
	if (skin->m_nBoneAniIndex < 0 || skin->m_nBoneAniIndex >= MAX_BONE_ANIMATION_LIST)
		return -1;
	const auto& bone = MeshManager::m_BoneAnimationList[skin->m_nBoneAniIndex];
	if (aniIndex < 0 || aniIndex >= 186 || static_cast<unsigned int>(aniIndex) >= bone.numAniTypeCount)
		return -1;
	return static_cast<int>(bone.numAniCut[aniIndex]);
}

int SelServerDemoClassIndex(const TMHuman* human)
{
	if (!human)
		return 0;
	int nClass = 0;
	if (human->m_sHeadIndex < 40 && (human->m_sHeadIndex % 10 == 1 || human->m_sHeadIndex % 10 > 5))
	{
		if (human->m_sHeadIndex % 10 == 1)
			nClass = human->m_sHeadIndex / 10;
		else
			nClass = human->m_sHeadIndex % 10 - 6;
		if (nClass > 3)
			nClass = 0;
	}
	return nClass;
...
int SelServerResolveBodyAniTable(const TMHuman* human, ECHAR_MOTION motion, bool mounted)
{
	if (!human)
		return -1;
	if (human->m_nSkinMeshType < 0 || human->m_nSkinMeshType >= MAX_ANI_TYPE)
		return -1;

	const int motionIndex = static_cast<int>(motion) + (mounted ? 28 : 0);
	if (motionIndex < 0 || motionIndex >= MAX_ANI_MOTION)
		return -1;

	if ((human->m_nSkinMeshType == 0 || human->m_nSkinMeshType == 1)
		&& human->m_sHeadIndex < 40
		&& (human->m_sHeadIndex % 10 == 1 || human->m_sHeadIndex % 10 > 5))
	{
		return static_cast<int>(g_MobAniTableEx[SelServerDemoClassIndex(human)][human->m_nSkinMeshType].dwAniTable[motionIndex]);
	}

	return static_cast<int>(g_MobAniTable[human->m_nSkinMeshType].dwAniTable[motionIndex]);
}

int SelServerResolveBodyClip(const TMHuman* human, int aniTable)
{
	if (!human)
		return -1;
	if (aniTable < 0 || aniTable >= MAX_ANI_MOTION)
		return -1;
	if (human->m_nSkinMeshType < 0 || human->m_nSkinMeshType >= MAX_ANI_TYPE)
		return -1;

	if (human->m_nSkinMeshType >= 0 && human->m_nSkinMeshType <= 5 && human->m_nSkinMeshType != 3)
	{
		if (human->m_nWeaponTypeIndex < 0 || human->m_nWeaponTypeIndex >= MAX_ANI_TYPE)
			return -1;
		return MeshManager::m_sAnimationArray[human->m_nSkinMeshType][human->m_nWeaponTypeIndex][aniTable];
	}

	return aniTable;
}

void WasmSelServerCaptureHumanTelemetry(TMSelectServerScene* scene)
{
	if (!scene)
		return;

	static unsigned int s_lastCaptureTime = 0;
	static unsigned int s_prevValid[50]{};
	static float s_prevPosX[50]{};
	static float s_prevPosY[50]{};
	static float s_lastDeltaX[50]{};
	static float s_lastDeltaY[50]{};
	const unsigned int serverTime = g_pTimerManager ? g_pTimerManager->GetServerTime() : 0;
	const bool advanceDelta = g_wydSelServerHumanTelemetryVersion == 0 || serverTime != s_lastCaptureTime;

	for (int i = 0; i < 50; ++i)
	{
		auto& out = g_wydSelServerHumanTelemetry[i];
		out = WydSelServerHumanTelemetry{};
		TMHuman* human = scene->m_pCheckHumanList[i];
		if (!human)
			continue;

		TMVector2 pos(human->m_vecPosition.x, human->m_vecPosition.y);
		const float groundMask = static_cast<float>(scene->GroundGetMask(pos));
		if (advanceDelta)
		{
			s_lastDeltaX[i] = s_prevValid[i] ? human->m_vecPosition.x - s_prevPosX[i] : 0.0f;
			s_lastDeltaY[i] = s_prevValid[i] ? human->m_vecPosition.y - s_prevPosY[i] : 0.0f;
			s_prevPosX[i] = human->m_vecPosition.x;
			s_prevPosY[i] = human->m_vecPosition.y;
			s_prevValid[i] = 1;
		}
		out.present = 1;
		out.posX = human->m_vecPosition.x;
		out.posY = human->m_vecPosition.y;
		out.deltaX = s_lastDeltaX[i];
		out.deltaY = s_lastDeltaY[i];
		out.wantHeight = human->m_fWantHeight;
		out.height = human->m_fHeight;
		out.groundMask = groundMask;
		out.groundHeight = groundMask * 0.1f;
		out.skinMeshType = human->m_nSkinMeshType;
		out.objType = static_cast<int>(human->m_dwObjType);
		out.visible = human->m_bVisible;
		out.mount = human->m_cMount;
		...
		out.bodyCurrentResolvedClip = SelServerResolveBodyClip(human, out.bodyCurrentTable);
		out.bodyMountedCurrentTable = SelServerResolveBodyAniTable(human, human->m_eMotion, true);
		out.bodyMountedCurrentResolvedClip = SelServerResolveBodyClip(human, out.bodyMountedCurrentTable);
		out.bodySeatingTable = SelServerResolveBodyAniTable(human, ECHAR_MOTION::ECMOTION_SEATING, false);
		out.bodySeatingResolvedClip = SelServerResolveBodyClip(human, out.bodySeatingTable);
		out.bodyMountedSeatingTable = SelServerResolveBodyAniTable(human, ECHAR_MOTION::ECMOTION_SEATING, true);
		out.bodyMountedSeatingResolvedClip = SelServerResolveBodyClip(human, out.bodyMountedSeatingTable);
		out.demoAni = scene->m_stDemoHuman[i].nHumanAni;
		out.mountPresent = human->m_pMount ? 1 : 0;
		out.moving = human->m_bMoveing;
		out.sliding = human->m_bSliding;
		out.lastRouteIndex = human->m_nLastRouteIndex;
		out.maxRouteIndex = human->m_nMaxRouteIndex;
		out.maxSpeed = human->m_fMaxSpeed;
		out.progressRate = human->m_fProgressRate;
		out.targetX = human->m_vecTargetPos.x;
		out.targetY = human->m_vecTargetPos.y;
		if (human->m_pSkinMesh)
		{
			out.skinX = human->m_pSkinMesh->m_vPosition.x;
			out.skinY = human->m_pSkinMesh->m_vPosition.y;
			out.skinZ = human->m_pSkinMesh->m_vPosition.z;
			out.skinAni = human->m_pSkinMesh->m_nAniIndex;
			out.skinFps = static_cast<int>(human->m_pSkinMesh->m_dwFPS);
			out.skinOffset = static_cast<int>(human->m_pSkinMesh->m_dwOffset);
			out.skinBoneAni = human->m_pSkinMesh->m_nBoneAniIndex;
			out.skinBaseMat = human->m_pSkinMesh->m_bBaseMat;
			out.skinAniBaseIndex = human->m_pSkinMesh->m_nAniBaseIndex;
			out.skinAniCut = SelServerAniCut(human->m_pSkinMesh, human->m_pSkinMesh->m_nAniIndex);
			out.skinGenerated = human->m_pSkinMesh->m_bMeshGenerated;
			out.skinFrameMeshes = WasmSelServerCountFrameMeshes(human->m_pSkinMesh->m_pRoot);
		}
		if (human->m_pMount)
		{
			out.mountAni = human->m_pMount->m_nAniIndex;
			out.mountFps = static_cast<int>(human->m_pMount->m_dwFPS);
			out.mountOffset = static_cast<int>(human->m_pMount->m_dwOffset);
			out.mountAniCut = SelServerAniCut(human->m_pMount, human->m_pMount->m_nAniIndex);
			out.mountGenerated = human->m_pMount->m_bMeshGenerated;
			out.mountFrameMeshes = WasmSelServerCountFrameMeshes(human->m_pMount->m_pRoot);
		}
	}
	if (advanceDelta)
		s_lastCaptureTime = serverTime;
	++g_wydSelServerHumanTelemetryVersion;
}
} // namespace
#endif
...
{
#if defined(__EMSCRIPTEN__)
	WasmSelServerLog("[selserver:init] begin");
#endif

	if (!LoadRC("UI\\SelServerScene2.txt"))
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init] LoadRC failed: UI\\SelServerScene2.bin");
#endif
		return 0;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init] control container missing");
#endif
		return 0;
	}
	...
	if (!m_pLoginPanel ||
		...
		!m_pLoginPanelText ||
		!m_pLoginIDText ||
		!m_pLoginPasswordText)
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init] required login controls missing");
#endif
		return 0;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init] scene text control missing");
#endif
		return 0;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init] edit controls missing");
#endif
		return 0;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLogPath("[selserver:init] LoadTileMap failed: ", szMapPath);
#endif
		if (m_bCriticalError)
	...
	if (m_pObjectContainerList[0]->Load(szDataPath))
	{
		...
		if (g_pSoundManager)
			...
			SwapLauncher();
#if defined(__EMSCRIPTEN__)
			WasmSelServerLog("[selserver:init] success");
#endif
			return 1;
		}
		else
		{
#if defined(__EMSCRIPTEN__)
			WasmSelServerLogPath("[selserver:init] object container load failed: ", szDataPath);
#endif
...
#if defined(__EMSCRIPTEN__)
	WasmSelServerCaptureHumanTelemetry(this);
#endif	
...
#if defined(__EMSCRIPTEN__)
	if ((wyd_d3d9_get_debug_flags() & kWydDebugDisableDemoCamera) != 0u)
	{
		pCamera->m_bStandAlone = 0;
		m_dwStartCamTime = 0;
		m_sPlayDemo = -1;
		return;
	}
...
void TMSelectServerScene::InitializeUI()
{
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init-ui] missing control container");
#endif
		return;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init-ui] P_SERVER_SEL missing");
#endif
		return;
	}
	...
	{
#if defined(__EMSCRIPTEN__)
		WasmSelServerLog("[selserver:init-ui] server list controls missing");
#endif
		return;
	}
