...
static void WydNormalizeTexturePath(const char* szIn, char* szOut, size_t nOutSize)
{
	if (szOut == nullptr || nOutSize == 0)
		return;
	...
	size_t n = 0;
	for (; szIn[n] != '\0' && n + 1 < nOutSize; ++n)
	{
		unsigned char c = static_cast<unsigned char>(szIn[n]);
		if (c == '/')
			c = '\\';
		if (c >= 'A' && c <= 'Z')
			c = static_cast<unsigned char>(c - 'A' + 'a');
		szOut[n] = static_cast<char>(c);
	}
	szOut[n] = '\0';
}

static bool WydTexturePathMatches(const char* szRequested, const char* szListed)
{
	char szReq[260]{};
	char szList[260]{};
	WydNormalizeTexturePath(szRequested, szReq, sizeof(szReq));
	WydNormalizeTexturePath(szListed, szList, sizeof(szList));
	return szReq[0] != '\0' && strcmp(szReq, szList) == 0;
}
...
int TextureManager::GetEffectTextureIndex(char* szTextureName)
{
	...
	{
		if (WydTexturePathMatches(szTextureName, m_stEffectTextureList[i].szFileName))
			return i;
	}

	char szLegacyName[260]{};
	sprintf_s(szLegacyName, "%s", szTextureName);
	int nStrLen = strlen(szLegacyName);
	for (int i = 0; i < MAX_EFFECT_TEXTURE; i++)
	{
		int nStrLen2 = strlen(m_stEffectTextureList[i].szFileName);
		sprintf((char*)&szLegacyName[nStrLen - 3],
			(char*)&m_stEffectTextureList[i].szFileName[nStrLen2 - 3]);

		if (!strcmp(szLegacyName, m_stEffectTextureList[i].szFileName))
			return i;
	}
...
int TextureManager::GetModelTextureIndex(char* szTextureName)
{
	...
	for (int i = 0; i < MAX_MODEL_TEXTURE; i++)
	{
		if (WydTexturePathMatches(szTextureName, m_stModelTextureList[i].szFileName))
			return i;
	}
	...
	for (int i = 0; i < MAX_MODEL_TEXTURE; i++)
	{
		...
		if (!strcmp(szLegacyName, m_stModelTextureList[i].szFileName))
			return i;
	}
