...
namespace {
std::unordered_map<const TMMesh*, std::string> g_wydMeshDebugNames;

const char* WydMeshDebugName(const TMMesh* mesh)
{
	auto it = g_wydMeshDebugNames.find(mesh);
	return it != g_wydMeshDebugNames.end() ? it->second.c_str() : "(unknown)";
}
} // namespace
#endif
...
{
#if defined(__EMSCRIPTEN__)
	g_wydMeshDebugNames.erase(this);
#endif
	if (m_pVB != nullptr)
	{
		m_pVB->Release();
...
HRESULT TMMesh::RenderDraw(int nTexOffset)
{
	for (size_t i = 0; i < m_dwAttCount; ++i)
	{
		...
		std::snprintf(
			debugLabel,
			sizeof(debugLabel),
			"TMMesh file=%s effect=%d fvf=%u attr=%u texIndex=%d faces=%u vertices=%u",
			WydMeshDebugName(this),
			m_bEffect,
			m_dwFVF,
			static_cast<unsigned int>(i),
			nTexOffset + m_nTextureIndex[i],
			static_cast<unsigned int>(m_AttRange[i].FaceCount),
			static_cast<unsigned int>(m_AttRange[i].VertexCount));
		wyd_d3d9_set_draw_label(debugLabel);
#endif
...
int TMMesh::LoadMsa(const char* szFileName)
{
	...
	if (!dot || _stricmp(dot, ".msa"))
	{
		LOG_WRITELOG("Invalid MSA path : %s\n", szFileName);
		return 0;
	}

#if defined(__EMSCRIPTEN__)
	g_wydMeshDebugNames[this] = szFileName;
#endif
	...
	if (m_dwAttCount == 0 || m_dwAttCount > 32)
	{
		LOG_WRITELOG("Invalid MSA attribute count : %s fvf=%u stride=%u att=%u\n", szFileName, m_dwFVF, m_sizeVertex, m_dwAttCount);
		fclose(fp);
		return 0;
	}
	...
	if (m_sizeVertex == 0 || vDesc.Size == 0 || (vDesc.Size % m_sizeVertex) != 0)
	{
		LOG_WRITELOG("Invalid MSA vertex buffer : %s stride=%u size=%u\n", szFileName, m_sizeVertex, vDesc.Size);
		fclose(fp);
		return 0;
	}
